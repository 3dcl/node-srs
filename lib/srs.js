...
function parse(arg) {
    ...
    if (result.name in oc(constants.merc_names)) {
        ...
        if (result.proj4 === '+proj=merc +lat_ts=0 +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +units=m +no_defs') {
           return force_merc(result);
        } else if (result.proj4 === '+proj=merc +lat_ts=0 +lon_0=0 +x_0=0 +y_0=0 +R=6378137 +units=m +no_defs') {
           return force_merc(result);
        }
        // break into parts
